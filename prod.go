...
func InsertUsuario(db *sql.DB, token string, nombre string, p0 string, p1 string, p2 string, p3 string, p4 string, p5 string, p6 string, p7 string, p8 string, p9 string) Response {
	...
	code := randSeq(32)
	resp := Response{}
	stmt, err := db.Prepare("INSERT INTO usuarios (user, pass, code, id_emp, p0, p1, p2, p3, p4, p5, p6, p7, p8, p9) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
	ErrorCheck(err)
	defer stmt.Close()
	pass := ""
	_, e := stmt.Exec(nombre, pass, code, GetIdEmp(token), p0, p1, p2, p3, p4, p5, p6, p7, p8, p9)
	ErrorCheck(e)
	if e == nil {
		SendEmail(code)
		resp.Op = 1
