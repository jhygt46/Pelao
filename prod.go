...
func main() {
	...
	//CreateMapImage("-33.4397852", "-70.6169508", 600, 300)
			...
			if listaPermisos.P0 {
				...
				if found {
					...
					if b {
						...
						m.Row(67, func() {
							m.Col(12, func() {
								_ = m.FileImage("./mapa.png", props.Rect{
									Center:  true,
									Percent: 100,
								})
							})
...
func InsertPropiedad(db *sql.DB, id_emp int, nombre string, lat string, lng string, rangopos string, comuna string, ciudad string, region string, pais string, direccion string, numero string, dominio string, dominio2 string, atencion_publico string, copropiedad string, destino string, detalle_destino string) (uint8, string, int) {
	...
	if nombre != "" {
		...
		if b1 && b2 && b3 && b4 {
			stmt, err := db.Prepare("INSERT INTO propiedades (nombre, lat, lng, rangopos, id_ciu, id_com, id_reg, id_pai, direccion, numero, dominio, dominio2, atencion_publico, copropiedad, destino, detalle_destino, id_emp) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
			ErrorCheck(err)
			defer stmt.Close()
			r, err := stmt.Exec(nombre, lat, lng, rangopos, id_ciu, id_com, id_reg, id_pai, direccion, numero, dominio, dominio2, atencion_publico, copropiedad, destino, detalle_destino, id_emp)
			ErrorCheck(err)
			if err == nil {
...
func UpdatePropiedad2A(db *sql.DB, id_emp int, id_pro int, id_rec int, sup_terreno string, posee_permiso_edificacion string, tipo_permiso_edificacion string, especificar string, numero_permiso_edificacion string, fecha_permiso string, cant_pisos_sobre_nivel string, cant_pisos_bajo_nivel string, superficie_edificada_sobre_nivel string, superficie_edificada_bajo_nivel string, aco_art_esp_transitorio string, recepcion_definitiva string, obra_p0 string, obra_p1 string, obra_p2 string, op0 string, op1 string, op2 string, op3 string, op4 string, op5 string, op6 string, op7 string, op8 string, op9 string, op10 string, op11 string, op12 string) (uint8, string) {
	...
	stmt, err := db.Prepare("UPDATE permiso_edificacion SET sup_terreno = ?, posee_permiso_edificacion = ?, tipo_permiso_edificacion = ?, especificar_tipo_permiso_edificacion = ?, numero_permiso = ?, fecha_permiso = ?, cant_pisos_sobre_nivel = ?, cant_pisos_bajo_nivel = ?, superficie_edificada_sobre_nivel = ?, superficie_edificada_bajo_nivel = ?, aco_art_esp_transitorio = ?, recepcion_definitiva = ?, obrap_faena = ?, obrap_grua = ?, obrap_excavacion = ?, op0 = ?, op1 = ?, op2 = ?, op3 = ?, op4 = ?, op5 = ?, op6 = ?, op7 = ?, op8 = ?, op9 = ?, op10 = ?, op11 = ?, op12 = ? WHERE id_rec = ? AND id_pro = ? AND id_emp = ?")
	_, err = stmt.Exec(sup_terreno, posee_permiso_edificacion, tipo_permiso_edificacion, especificar, numero_permiso_edificacion, fecha_permiso, cant_pisos_sobre_nivel, cant_pisos_bajo_nivel, superficie_edificada_sobre_nivel, superficie_edificada_bajo_nivel, aco_art_esp_transitorio, recepcion_definitiva, obra_p0, obra_p1, obra_p2, op0, op1, op2, op3, op4, op5, op6, op7, op8, op9, op10, op11, op12, id_rec, id_pro, id_emp)
	if err == nil {
		return 1, "Empresa actualizada correctamente"
	} else {
		return 2, "La empresa no pudo ser actualizada"
...
func InsertPropiedad2A(db *sql.DB, id_emp int, id_pro int, sup_terreno string, posee_permiso_edificacion string, tipo_permiso_edificacion string, especificar string, numero_permiso_edificacion string, fecha_permiso string, cant_pisos_sobre_nivel string, cant_pisos_bajo_nivel string, superficie_edificada_sobre_nivel string, superficie_edificada_bajo_nivel string, aco_art_esp_transitorio string, recepcion_definitiva string, obra_p0 string, obra_p1 string, obra_p2 string, op0 string, op1 string, op2 string, op3 string, op4 string, op5 string, op6 string, op7 string, op8 string, op9 string, op10 string, op11 string, op12 string) (uint8, string, int) {
	...
	stmt, err := db.Prepare("INSERT INTO permiso_edificacion (sup_terreno, posee_permiso_edificacion, tipo_permiso_edificacion, especificar_tipo_permiso_edificacion, numero_permiso, fecha_permiso, cant_pisos_sobre_nivel, cant_pisos_bajo_nivel, superficie_edificada_sobre_nivel, superficie_edificada_bajo_nivel, aco_art_esp_transitorio, recepcion_definitiva, obrap_faena, obrap_grua, obrap_excavacion, op0, op1, op2, op3, op4, op5, op6, op7, op8, op9, op10, op11, op12, id_pro, id_emp) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
	ErrorCheck(err)
	defer stmt.Close()
	r, err := stmt.Exec(sup_terreno, posee_permiso_edificacion, tipo_permiso_edificacion, especificar, numero_permiso_edificacion, fecha_permiso, cant_pisos_sobre_nivel, cant_pisos_bajo_nivel, superficie_edificada_sobre_nivel, superficie_edificada_bajo_nivel, aco_art_esp_transitorio, recepcion_definitiva, obra_p0, obra_p1, obra_p2, op0, op1, op2, op3, op4, op5, op6, op7, op8, op9, op10, op11, op12, id_pro, id_emp)
	ErrorCheck(err)
	if err == nil {
		id, err := r.LastInsertId()
		if err == nil {
...
func CreateMapImage(lat string, lng string, width int, height int) {

	url := fmt.Sprintf("https://maps.googleapis.com/maps/api/staticmap?center=%v,%v&zoom=13&size=%vx%v&maptype=roadmap&key=%v", lat, lng, width, height, pass.Passwords.Gmapkey)
