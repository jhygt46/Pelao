...
func (h *MyHandler) StartDaemon() {
	...
	t1 := time.Since(pass.Conf.Lapsed)
	if t1.Seconds() > 60 {
		if pass.Conf.Alert == 0 {
			t2 := time.Since(pass.Conf.LastAlert)
			if pass.Conf.LastAlert.IsZero() || t2.Seconds() > 86400 {
				pass.Conf.Alert = 1
				pass.Conf.LastAlert = time.Now()
				// ENVIAR EMAIL
				fmt.Println("ENVIAR EMAIL")
			}
		}
	} else {
		if pass.Conf.Alert == 1 {
			pass.Conf.Alert = 0
		}
	}
}
