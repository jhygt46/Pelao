...
type CamposPropiedades struct {
	...
	Zona_normativa               string `json:"Zona_normativa"`
	Usos_permitidos              string `json:"Usos_permitidos"`
	Usos_prohibidos              string `json:"Usos_prohibidos"`
	Densidad                     string `json:"Densidad"`
	Coef_constructibilidad       string `json:"Coef_constructibilidad"`
	Coef_ocupacion_suelo         string `json:"Coef_ocupacion_suelo"`
...
type PermisoEdificacion struct {
	...
	Archivos                             map[int][]ArchivosPermisoEdificacion `json:"Recepcion_Definitiva"`
	...
	case "guardar_propiedad1":
		...
		if found, id_emp, listaPermisos := Permisos(token); found {
			if listaPermisos.P0 {
				...
				if id > 0 {
					resp.Op, resp.Msg = UpdatePropiedad(db, id_emp, id, nombre, lat, lng, comuna, ciudad, region, pais, direccion, numero, dominio, dominio2, atencion_publico, copropiedad, destino, detalle_destino)
				}
				if id == 0 {
					resp.Op, resp.Msg, id = InsertPropiedad(db, id_emp, nombre, lat, lng, comuna, ciudad, region, pais, direccion, numero, dominio, dominio2, atencion_publico, copropiedad, destino, detalle_destino)
				}
	...
	case "guardar_propiedad2A":
		...
		if found, id_emp, listaPermisos := Permisos(token); found {
			if listaPermisos.P0 {
				...
				if id_rec > 0 {
					resp.Op, resp.Msg = UpdatePropiedad2A(db, id_emp, id, id_rec, sup_terreno, posee_permiso_edificacion, tipo_permiso_edificacion, especificar, numero_permiso_edificacion, fecha_permiso, cant_pisos_sobre_nivel, cant_pisos_bajo_nivel, superficie_edificada_sobre_nivel, superficie_edificada_bajo_nivel, aco_art_esp_transitorio, recepcion_definitiva)
				}
				if id_rec == 0 {
					resp.Op, resp.Msg, id_rec = InsertPropiedad2A(db, id_emp, id, sup_terreno, posee_permiso_edificacion, tipo_permiso_edificacion, especificar, numero_permiso_edificacion, fecha_permiso, cant_pisos_sobre_nivel, cant_pisos_bajo_nivel, superficie_edificada_sobre_nivel, superficie_edificada_bajo_nivel, aco_art_esp_transitorio, recepcion_definitiva)
				}

				files, err := ctx.MultipartForm()
				if err == nil {
					errup1, list1 := UploadFile(fmt.Sprintf("./files/images/%v/1", id_emp), files.File["permiso"], false, []string{"PDF"}, "")
					if errup1 {
						for _, x := range list1 {
							SaveFileDb2(db, x, "", 1, "", "", id_rec, id, id_emp)
						}
					}
					errup2, list2 := UploadFile(fmt.Sprintf("./files/images/%v/2", id_emp), files.File["plano"], false, []string{"PDF"}, "")
					if errup2 {
						for _, x := range list2 {
					...
					errup3, list3 := UploadFile(fmt.Sprintf("./files/images/%v/3", id_emp), files.File["doc_recepcion_parcial"], false, []string{"PDF"}, "")
					errup4, list4 := UploadFile(fmt.Sprintf("./files/images/%v/4", id_emp), files.File["doc2_recepcion_parcial"], false, []string{"PDF"}, "")
					...
					errup5, list5 := UploadFile(fmt.Sprintf("./files/images/%v/5", id_emp), files.File["doc_recepcion_total"], false, []string{"PDF"}, "")
					errup6, list6 := UploadFile(fmt.Sprintf("./files/images/%v/6", id_emp), files.File["doc2_recepcion_total"], false, []string{"PDF"}, "")
		...
		if found, id_emp, listaPermisos := Permisos(token); found {
			if listaPermisos.P0 {
				...
				if id > 0 {
					...
					if err == nil {
						errup1, list1 := UploadFile(fmt.Sprintf("./files/pdf/%v/1", id_emp), files.File["doc_electrico_te1"], false, []string{"PDF"}, "")
	...
	case "guardar_propiedad8":
		...
		if found, id_emp, listaPermisos := Permisos(token); found {
			if listaPermisos.P0 {
				...
				if id > 0 {
					resp.Op, resp.Msg = UpdatePropiedad8(db, id_emp, id, cert_info_previas, tipo_instrumento, especificar_tipo_instrumento, normativo_destino, zona_normativa, usos_permitidos, usos_prohibidos, densidad, coef_constructibilidad, coef_ocupacion_suelo)
					files, err := ctx.MultipartForm()
					if err == nil {
...
func CreateExcel(id_emp int, lista []int) (*bytes.Buffer, error) {
	...
	*/
	...
	for i, x := range lista {
		pro, found := GetPropiedad(id_emp, x)
		if found {
			...
			f.SetCellValue("Sheet1", fmt.Sprintf("%v%v", listaLetras[j], i+m), pro.Especificar_tipo_instrumento) //ay
			f.SetCellValue("Sheet1", fmt.Sprintf("%v%v", listaLetras[j], i+m), pro.Normativo_destino)            //az
			f.SetCellValue("Sheet1", fmt.Sprintf("%v%v", listaLetras[j], i+m), pro.Zona_normativa)               //ba
			...
			f.SetCellValue("Sheet1", fmt.Sprintf("%v%v", listaLetras[j], i+m), pro.Densidad)                     //bd
			f.SetCellValue("Sheet1", fmt.Sprintf("%v%v", listaLetras[j], i+m), pro.Coef_constructibilidad)       //be
			f.SetCellValue("Sheet1", fmt.Sprintf("%v%v", listaLetras[j], i+m), pro.Coef_ocupacion_suelo)         //bf
...
func Pages(ctx *fasthttp.RequestCtx) {
	...
	case "crearPropiedad":
		...
		if found, id_emp, listaPermisos := Permisos(token); found {
			...
			if id > 0 {
				aux, found := GetPropiedad(id_emp, id)
				if found {
	...
	case "crearPropiedad2PermisoEdificacion":
		...
		if found, id_emp, listaPermisos := Permisos(token); found {
			...
			if id > 0 {
				...
				CamposPropiedades, Lista, found := GetPropiedad2A(id_emp, id_rec)
				if found {
...
func GetPropiedad(id_emp int, id int) (CamposPropiedades, bool) {
...
func GetPropiedad2A(id_emp int, id_rec int) (CamposPropiedades, []Lista, bool) {

	aux := CamposPropiedades{}
	data := CamposPropiedades{}
	lista := []Lista{}
	...
	defer db.Close()
	if err != nil {
		return data, lista, false
	}

	cn := 0
	res, err := db.Query("SELECT id_rec, sup_terreno, posee_permiso_edificacion, tipo_permiso_edificacion, especificar_tipo_permiso_edificacion, numero_permiso, fecha_permiso, cant_pisos_sobre_nivel, cant_pisos_bajo_nivel, superficie_edificada_sobre_nivel, superficie_edificada_bajo_nivel, aco_art_esp_transitorio, recepcion_definitiva FROM permiso_edificacion WHERE eliminado = ? AND id_emp = ?", cn, id_emp)
	defer res.Close()
	if err != nil {
		return data, lista, false
	}
	for res.Next() {
		err := res.Scan(&aux.PermisosEdificacionIndex.Id_rec, &aux.PermisosEdificacionIndex.Sup_Terreno, &aux.PermisosEdificacionIndex.Posee_Permiso_Edificacion, &aux.PermisosEdificacionIndex.Tipo_Permiso_Edificacion, &aux.PermisosEdificacionIndex.Especificar_Tipo_Permiso_Edificacion, &aux.PermisosEdificacionIndex.Numero_Permiso, &aux.PermisosEdificacionIndex.Fecha_Permiso, &aux.PermisosEdificacionIndex.Cant_Pisos_Sobre_Nivel, &aux.PermisosEdificacionIndex.Cant_Pisos_Bajo_Nivel, &aux.PermisosEdificacionIndex.Sup_Edificada_Sobre_Nivel, &aux.PermisosEdificacionIndex.Sup_Edificada_Sobre_Nivel, &aux.PermisosEdificacionIndex.Aco_Art_Esp_Transitorio, &aux.PermisosEdificacionIndex.Recepcion_Definitiva)
		if err != nil {
			return data, lista, false
		}
		if aux.PermisosEdificacionIndex.Id_rec == id_rec {
			data.PermisosEdificacionIndex = aux.PermisosEdificacionIndex
		}
		lista = append(lista, Lista{Id: aux.PermisosEdificacionIndex.Id_rec, Nombre: NombrePermiso(aux.PermisosEdificacionIndex.Tipo_Permiso_Edificacion, aux.PermisosEdificacionIndex.Especificar_Tipo_Permiso_Edificacion)})
		auxlista, ultimos_archivos, auxfound := ArchivosPermisosEdificacion(aux.PermisosEdificacionIndex.Id_rec)
		if auxfound {
			data.PermisosEdificacionIndex.Archivos = auxlista
			data.PermisosEdificacionIndex.UltimosArchivos = ultimos_archivos
		}
	}
	return data, lista, true
}
...
func InsertPropiedad(db *sql.DB, id_emp int, nombre string, lat string, lng string, comuna string, ciudad string, region string, pais string, direccion string, numero string, dominio string, dominio2 string, atencion_publico string, copropiedad string, destino string, detalle_destino string) (uint8, string, int) {
...
func UpdatePropiedad(db *sql.DB, id_emp int, id int, nombre string, lat string, lng string, comuna string, ciudad string, region string, pais string, direccion string, numero string, dominio string, dominio2 string, atencion_publico string, copropiedad string, destino string, detalle_destino string) (uint8, string) {
...
func UpdatePropiedad2A(db *sql.DB, id_emp int, id_pro int, id_rec int, sup_terreno string, posee_permiso_edificacion string, tipo_permiso_edificacion string, especificar string, numero_permiso_edificacion string, fecha_permiso string, cant_pisos_sobre_nivel string, cant_pisos_bajo_nivel string, superficie_edificada_sobre_nivel string, superficie_edificada_bajo_nivel string, aco_art_esp_transitorio string, recepcion_definitiva string) (uint8, string) {

	stmt, err := db.Prepare("UPDATE permiso_edificacion SET sup_terreno = ?, posee_permiso_edificacion = ?, tipo_permiso_edificacion = ?, especificar_tipo_permiso_edificacion = ?, numero_permiso = ?, fecha_permiso = ?, cant_pisos_sobre_nivel = ?, cant_pisos_bajo_nivel = ?, superficie_edificada_sobre_nivel = ?, superficie_edificada_bajo_nivel = ?, aco_art_esp_transitorio = ?, recepcion_definitiva = ? WHERE id_rec = ? AND id_pro = ? AND id_emp = ?")
	_, err = stmt.Exec(sup_terreno, posee_permiso_edificacion, tipo_permiso_edificacion, especificar, numero_permiso_edificacion, fecha_permiso, cant_pisos_sobre_nivel, cant_pisos_bajo_nivel, superficie_edificada_sobre_nivel, superficie_edificada_bajo_nivel, aco_art_esp_transitorio, recepcion_definitiva, id_rec, id_pro, id_emp)
	if err == nil {
		return 1, "Empresa actualizada correctamente"
	} else {
		return 2, "La empresa no pudo ser actualizada"
...
func InsertPropiedad2A(db *sql.DB, id_emp int, id_pro int, sup_terreno string, posee_permiso_edificacion string, tipo_permiso_edificacion string, especificar string, numero_permiso_edificacion string, fecha_permiso string, cant_pisos_sobre_nivel string, cant_pisos_bajo_nivel string, superficie_edificada_sobre_nivel string, superficie_edificada_bajo_nivel string, aco_art_esp_transitorio string, recepcion_definitiva string) (uint8, string, int) {

	stmt, err := db.Prepare("INSERT INTO permiso_edificacion (sup_terreno, posee_permiso_edificacion, tipo_permiso_edificacion, especificar_tipo_permiso_edificacion, numero_permiso, fecha_permiso, cant_pisos_sobre_nivel, cant_pisos_bajo_nivel, superficie_edificada_sobre_nivel, superficie_edificada_bajo_nivel, aco_art_esp_transitorio, recepcion_definitiva, id_pro, id_emp) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
	defer stmt.Close()
	r, err := stmt.Exec(sup_terreno, posee_permiso_edificacion, tipo_permiso_edificacion, especificar, numero_permiso_edificacion, fecha_permiso, cant_pisos_sobre_nivel, cant_pisos_bajo_nivel, superficie_edificada_sobre_nivel, superficie_edificada_bajo_nivel, aco_art_esp_transitorio, recepcion_definitiva, id_pro, id_emp)
	if err == nil {
		id, err := r.LastInsertId()
		if err == nil {
			return 1, "Propiedad ingresada correctamente", int(id)
...
func UpdatePropiedad3(db *sql.DB, id_emp int, id int, electrico_te1 string, dotacion_ap string, dotacion_alcance string, instalacion_ascensor string, te1_ascensor string, certificado_ascensor string, clima string, seguridad_incendio string) (uint8, string, uint8) {
	...
	_, err = stmt.Exec(electrico_te1, dotacion_ap, dotacion_alcance, instalacion_ascensor, te1_ascensor, certificado_ascensor, clima, seguridad_incendio, id, id_emp)
	if err == nil {
...
func UpdatePropiedad4(db *sql.DB, id_emp int, id int, tasacion_valor_comercial string, ano_tasacion string, contrato_arriendo string, contrato_subarriendo string) (uint8, string) {
	...
	stmt, err := db.Prepare("UPDATE propiedades SET tasacion_valor_comercial = ?, ano_tasacion = ?, contrato_arriendo = ?, contrato_subarriendo = ? WHERE id_pro = ? AND id_emp = ?")
	_, err = stmt.Exec(tasacion_valor_comercial, ano_tasacion, contrato_arriendo, contrato_subarriendo, id, id_emp)
	if err == nil {
		return 1, "Empresa actualizada correctamente"
	} else {
		return 2, "La empresa no pudo ser actualizada"
...
func UpdatePropiedad5(db *sql.DB, id_emp int, id int, domnompropietario string, posee_gp string, posee_ap string) (uint8, string) {
	...
	stmt, err := db.Prepare("UPDATE propiedades SET nompropietarioconservador = ?, posee_gp = ?, posee_ap = ? WHERE id_pro = ? AND id_emp = ?")
	_, err = stmt.Exec(domnompropietario, posee_gp, posee_ap, id, id_emp)
	if err == nil {
		return 1, "Empresa actualizada correctamente"
	} else {
		return 2, "La empresa no pudo ser actualizada"
...
func UpdatePropiedad6(db *sql.DB, id_emp int, id int, fiscal_serie string, fiscal_destino string, rol_manzana string, rol_predio string, fiscal_exento string, avaluo_fiscal string, contribucion_fiscal string, superficie_terreno string, superficie_edificada string, superficie_pavimentos string) (uint8, string) {
	...
	stmt, err := db.Prepare("UPDATE propiedades SET fiscal_serie = ?, fiscal_destino = ?, rol_manzana = ?, rol_predio = ?, fiscal_exento = ?, fiscal_avaluo = ?, fiscal_contribucion = ?, fiscal_sup_terreno = ?, fiscal_sup_edificada = ?, fiscal_sup_pavimentos = ? WHERE id_pro = ? AND id_emp = ?")
	_, err = stmt.Exec(fiscal_serie, fiscal_destino, rol_manzana, rol_predio, fiscal_exento, avaluo_fiscal, contribucion_fiscal, superficie_terreno, superficie_edificada, superficie_pavimentos, id, id_emp)
	if err == nil {
		return 1, "Empresa actualizada correctamente"
	} else {
		return 2, "La empresa no pudo ser actualizada"
...
func UpdatePropiedad7(db *sql.DB, id_emp int, id int, valor_terreno string, valor_edificacion string, valor_obras_complementarias string, valor_total string) (uint8, string) {
	...
	_, err = stmt.Exec(valor_terreno, valor_edificacion, valor_obras_complementarias, valor_total, id, id_emp)
	if err == nil {
		return 1, "Empresa actualizada correctamente"
	} else {
		return 2, "La empresa no pudo ser actualizada"
...
func UpdatePropiedad8(db *sql.DB, id_emp int, id int, cert_info_previas string, tipo_instrumento string, especificar_tipo_instrumento string, normativo_destino string, zona_normativa string, usos_permitidos string, usos_prohibidos string, densidad string, coef_constructibilidad string, coef_ocupacion_suelo string) (uint8, string) {

	stmt, err := db.Prepare("UPDATE propiedades SET cert_info_previas = ?, tipo_instrumento = ?, especificar_tipo_instrumento = ?, normativo_destino = ?, zona_normativa = ?, usos_permitidos = ?, usos_prohibidos = ?, densidad = ?, coef_constructibilidad = ?, coef_ocupacion_suelo = ? WHERE id_pro = ? AND id_emp = ?")
	_, err = stmt.Exec(cert_info_previas, tipo_instrumento, especificar_tipo_instrumento, normativo_destino, zona_normativa, usos_permitidos, usos_prohibidos, densidad, coef_constructibilidad, coef_ocupacion_suelo, id, id_emp)
	if err == nil {
		return 1, "Empresa actualizada correctamente"
	} else {
		return 2, "La empresa no pudo ser actualizada"
...
func SaveFileDb(db *sql.DB, nombre string, fojas string, numero string, fecha string, ano string, tipo int, valor_arriendo string, renovacion_auto string, tipo_de_plano string, id_pro int, id_emp int) bool {
	...
	ErrorCheck(err)
	defer stmt.Close()
	_, err = stmt.Exec(nombre, fojas, numero, fecha, ano, tipo, valor_arriendo, renovacion_auto, tipo_de_plano, id_pro, id_emp)
	ErrorCheck(err)
	if err == nil {
		return true
	} else {
...
func CreateMapImage() {
	...
	gmapkey := "AIzaSyCIXenuYoczpO6oh4uzeOj11b7Nvg8zrFM"
	url := fmt.Sprintf("https://maps.googleapis.com/maps/api/staticmap?center=40.714728,-73.998672&zoom=13&size=600x300&maptype=roadmap&key=%v", gmapkey)
	...
	fmt.Println("La imagen del mapa se ha guardado correctamente.")
}
